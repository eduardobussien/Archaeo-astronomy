...
| Epoch | Engine | Notes |
|---|---|---|
| After ~4800 BC | **astropy** | Full ICRS → GCRS → AltAz transform with proper motion applied at J2000 + Δt |
| Before ~4800 BC | **Meeus/Lieske** | Manual precession matrix (IAU 1976 + Lieske 1977 zeta/theta/z); same proper-motion offset |

Stellar proper motion is applied by nudging J2000 RA/Dec by `pm × Δt` before the coordinate transform, giving physically accurate star positions back at least 10,000 years.
...
**Heliacal rising** is found by scanning 460 days starting from the prior October, checking each dawn (binary search on Sun altitude = arc-vision threshold, default −10°) whether the target star is above the horizon and the Sun is below it.
...
| `GET /api/sites` | — | List of all monument sites with coordinates and orientation notes |
